...
export function ensureElements<T extends HTMLElement>(
	...
	if (Array.isArray(selectorElement)) return selectorElement;
...
export function setElementData<T extends Record<string, unknown> | object>(
	...
	for (const key in data) el.dataset[key] = String(data[key]);
}
...
Здесь не учтено много факторов в Интернете можно найти более полные реализации */
